...
class FisClient {
  ...
  async nearby(lat, lon, limit = 12) {
    const y = Number(lat);
    const x = Number(lon);
    const wanted = Math.min(Math.max(Number(limit) || 12, 1), 25);
    if (!Number.isFinite(y) || !Number.isFinite(x)) throw new Error('Invalid Homey location');

    const groups = [];
    for (const radiusDeg of [0.02, 0.05, 0.10, 0.20]) {
      let found = [];
      try { found = await this._searchBridgesNear(x, y, radiusDeg, 50); }
      catch (err) { if (!groups.length && radiusDeg === 0.20) throw err; }
      groups.push(found);
      const merged = this._mergeCandidates(groups);
      if (merged.length >= wanted) break;
    }

    return this._mergeCandidates(groups)
      .map(item => ({ ...item, distanceMeters: distanceMeters(y, x, item.lat, item.lon) }))
      .filter(item => Number.isFinite(item.distanceMeters))
      .sort((a, b) => a.distanceMeters - b.distanceMeters || a.name.localeCompare(b.name))
      .slice(0, wanted)
      .map(item => ({ ...item, candidateId: `${item.fisId}:${item.isrsId}` }));
  }
